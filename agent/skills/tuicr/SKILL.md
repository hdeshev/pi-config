---
name: tuicr
description: Review local git changes with tuicr TUI via zellij split pane
---

# tuicr - TUI Change Reviewer

Launch the `tuicr` TUI tool in a zellij split pane to interactively review local git changes.
...
1. Detects if the current agent session is running inside zellij
2. If yes: Creates a split pane with tuicr running in it
3. If no: Provides instructions to restart the agent inside zellij
...
3. **Handle the result**:
   ...
   - If not in zellij: relay the instructions to the user
...
```bash
TUICR_PANE_POSITION=bottom <skill-directory>/tuicr-wrapper.sh /path/to/repo
```
...
- Switch between panes: `Alt + ←↑↓→` arrows (if your terminal supports it) or `Ctrl + p` then arrow keys
- Close tuicr: Press `q` in tuicr (pane closes automatically)
- Resize panes: `Ctrl + p` then `r` to enter resize mode, then arrow keys
- Zoom current pane: `Ctrl + p` then `f` (toggle)
